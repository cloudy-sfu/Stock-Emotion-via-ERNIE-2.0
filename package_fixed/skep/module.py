...
@moduleinfo(
    ...
    version="1.0.0",
    summary=
    "SKEP: Sentiment Knowledge Enhanced Pre-training for Sentiment Analysis. Ernie_skep_sentiment_analysis module is "
    "initialize with enie_1.0_chn_large when pretraining. This module is finetuned on ChnSentiCorp dataset to do "
    "sentiment claasification. It can do sentiment analysis prediction directly, label as positive or negative.",
    author="baidu-nlp",
    author_email="",
...
class ErnieSkepSentimentAnalysis(TransformerModule):
    ...
    def predict_sentiment(self, texts=[], use_gpu=False):
        ...
        #     except:
        #         raise RuntimeError(
        #             "Environment Variable CUDA_VISIBLE_DEVICES is not set correctly. If you wanna use gpu, please set "
...
if __name__ == '__main__':
    ...
    test_texts = ['ä½ ä¸æ˜¯ä¸èªæ˜Žï¼Œè€Œæ˜¯ä¸è®¤çœŸ', 'è™½ç„¶å°æ˜Žå¾ˆåŠªåŠ›ï¼Œä½†æ˜¯ä»–è¿˜æ˜¯æ²¡æœ‰è€ƒ100åˆ†']
    results = test_module.predict_sentiment(test_texts, use_gpu=False)
    print(results)
    test_module.context(max_seq_len=128)
    print(test_module.get_embedding(texts=[['ä½ ä¸æ˜¯ä¸èªæ˜Žï¼Œè€Œæ˜¯ä¸è®¤çœŸ']]))
